...
def CNN_new(train_x, train_y, validation_x, validation_y):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('accuracy')
    plt.title('Data-resampled-color', loc='center')
    plt.legend(loc='lower right')
    plt.show()

    val_loss, val_accuracy = model.evaluate(validation_x, validation_y, verbose=2)
...
#splitting the dataset, if data augmentation is used: change the X and y to dataset_x and dataset_y
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.20, random_state=420)


CNN_new(X_train, y_train, X_test, y_test)