...
Physio- and manual therapists treat patients that have limitations in mobility of their shoulder joint. Knowing the exact limitations, and knowing the changes in these limitations is important for the therapists while helping patients in revalidation processes.

The measurement of the mobility of the shoulder joint is either done manually (protractor) or with a sensor system (‘flock of birds’). The second is to expensive (time consuming) to use every time, the first is inaccurate.

Long term goal: Find an easy, accurate measurement method.

Short term: Is it possible to cluster patients in groups with similar complaints and / or similar diagnosis based on the flock of birds data? What parameters are used for this clustering?”
